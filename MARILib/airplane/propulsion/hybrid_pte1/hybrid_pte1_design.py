...
def eval_hybrid_engine_design(aircraft):
    ...
    for rating in propulsion.rating_code:
        ...
        (fn,data) = propu.turbofan_thrust(aircraft,Pamb,Tamb,fd_mach[rating],rating,fd_nei[rating])
    ...
    (fn,data) = propu.turbofan_thrust(aircraft,Pamb,Tamb,fd_mach[MTO],MTO,fd_nei[MTO])
    (fn_core,fn_fan0,fn0,shaft_power0) = data

    shaft_power1 = (1-e_power_ratio[MTO])*shaft_power0     # Shaft power dedicated to the fan
...
def eval_hybrid_nacelle_design(aircraft):
    ...
    kSize = math.sqrt(engine.kfn_off_take)      # Diameter decrease due to max thrust decrease
...
#===========================================================================================================
def eval_boundary_layer(body_width,hub_width):
    """
    ...
    def fct_specific_flows(d1,r1,d0,r0):
        (q0s0,q1s0,q2s0,v1s0,dvs0) = craft_aero.specific_air_flows(r0,d0,d0)
        (q0s1,q1s1,q2s1,v1s1,dvs1) = craft_aero.specific_air_flows(r1,d1,d1)
        y = q2s0 - q2s1
        return y
    #-----------------------------------------------------------------------------------------------------------
...
def eval_bli_nacelle_design(this_nacelle,Pamb,Tamb,Mach,shaft_power,hub_width,body_length,body_width):
    ...
    #-----------------------------------------------------------------------------------------------------------
    r0 = 0.5*body_width     # Radius of the fuselage, supposed constant
    d0 = craft_aero.boundary_layer(Re,body_length)     # theoritical thickness of the boundary layer without taking account of fuselage tapering
    r1 = 0.5*hub_width        # Radius of the hub of the efan nacelle
    ...
    CQoA1 = craft_aero.corrected_air_flow(Ptot,Ttot,MachFan)        # Corrected air flow per area at fan position

    eFanArea = q1/CQoA1     # Fan area around the hub

    fan_width = math.sqrt(hub_width**2 + 4*eFanArea/math.pi)        # Fan diameter
    ...
    CQoA2 = craft_aero.corrected_air_flow(PtotJet,TtotJet,MachJet)     # Corrected air flow per area at nozzle output
