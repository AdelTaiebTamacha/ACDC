...
def draw_3d_view(aircraft,window_title,plot_title):
    ...
    if (aircraft.propulsion.architecture==1):
        ...
        nac_length = aircraft.turboprop_nacelle.length
        nac_height = aircraft.turboprop_nacelle.width
        nac_width = aircraft.turboprop_nacelle.width
        nac_x_ext = aircraft.turboprop_nacelle.x_ext
        nac_y_ext = aircraft.turboprop_nacelle.y_ext
        nac_z_ext = aircraft.turboprop_nacelle.z_ext
    else:
    ...
    htp_yz = np.array([[ 0           , htp_z_axe-0.5*fus_height                                                           ] ,
                       [ 0.5*htp_span , htp_z_axe-0.5*fus_height+0.5*htp_span*math.tan(htp_dihedral)                           ] ,
                       [ 0.5*htp_span , htp_z_axe-0.5*fus_height+0.5*htp_span*math.tan(htp_dihedral)-htp_t_o_c*htp_c_tip ] ,
